...
本文说明在 **Mac（Apple Silicon / Intel）** 上部署 **Family Media Center**（GitHub：[family-mediacenter](https://github.com/lhp2048/family-mediacenter)）的完整流程：应用本身、下载组件、Docker 可选服务、以及 **launchd 常驻**。

独立 `git clone` 后目录名为 `family-mediacenter`；在 smart-family monorepo 内则为 `family_mediacenter`。

## 支持情况概览

| 项目 | macOS 支持 | 说明 |
|------|------------|------|
| Family Media Center 主服务 | ✅ | Python 跨平台；已有 `start.command`、`scripts/start.sh` |
...
| launchd 常驻 | ✅ | 模板见 `deploy/family-mediacenter.plist` |
...
download:
  default_dir: downloads # 或绝对路径，如 /Users/you/Data/family-mediacenter
  max_concurrent: 3
...
```bash
PROJECT_ROOT="$(pwd)"
sed "s|__PROJECT_ROOT__|${PROJECT_ROOT}|g" deploy/family-mediacenter.plist \
  > ~/Library/LaunchAgents/com.family.mediacenter.service.plist
```

3. 加载服务：

```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.family.mediacenter.service.plist
launchctl kickstart -k gui/$(id -u)/com.family.mediacenter.service
```

4. 查看状态 / 日志：

```bash
launchctl list | grep family.mediacenter
...
```text
/Users/你的用户名/family-mediacenter/.venv/bin/yt-dlp
```
...
```bash
~/family-mediacenter/.venv/bin/yt-dlp --version
~/family-mediacenter/.venv/bin/python --version
```
...
```yaml
engines:
  ytdlp_path: /Users/muxin/family-mediacenter/.venv/bin/yt-dlp
```
...
- WorkBuddy Skill：`cp -r skills/family-mediacenter ~/.workbuddy/skills/family-mediacenter`
- Cursor MCP：在 MCP 配置中将路径改为 Mac 上的项目绝对路径，并设置 `FAMILY_MEDIACENTER_URL`
